...
my_env.close_env()
print(f'training stage cost time: {time.time() - t}')

print(f'--------- {args.suffix} best epoch {my_session.best_epoch}------------')
for top_k in eval(args.topk):
    print(f'hr@{top_k} = {my_session.best_hr[top_k]:.5f}, recall@{top_k} = {my_session.best_recall[top_k]:.5f}, ndcg@{top_k} = {my_session.best_ndcg[top_k]:.5f}')
print(f'--------- {args.suffix} test------------')
for top_k in eval(args.topk):
    print(f'hr@{top_k} = {my_session.test_hr[top_k]:.5f}, recall@{top_k} = {my_session.test_recall[top_k]:.5f}, ndcg@{top_k} = {my_session.test_ndcg[top_k]:.5f}')
